...
require(MASS)
#repcw01 <- repcw01 %>%  mutate(p_r_cw = revalue(rep_civwar_DV, c("0" = "Peace","1" = "Repression","2" = "Civil War")))
#repcw01$p_r_cw <- ordered(as.factor(repcw01$p_r_cw))
#save(repcw01, file = "repcw01.RData")
model.repCivWar.01 <- polr(p_r_cw ~ logGDPpc + parliament + major_oil + major_primary + disaster,
             data = repcw01,
             Hess=TRUE, 
             method="logistic"); summary(model.repCivWar.01)
...
if(F){
ggplot(repcw01, aes(x=logGDPpc, fill=p_r_cw)) +
  geom_histogram(binwidth=.5, alpha=.5, position="identity")
} # this is a histogram plot

# View Data
cdplot(repcw01$p_r_cw~repcw01$logGDPpc,ylab = "Peace,Repression,&CivilWar",xlab = "logGDP")
    ...
    Xb.1[j] <-  sim.coefs[i,6]*1 +                               # Cutpoint 6   Identify these from sim.coefs
                sim.coefs[i,7]*0 -                               # Cutpoint 7   Identify these from sim.coefs
                  (sim.coefs[i,1]*as.numeric(logGDPpc) +                              # 1
                    ...
                    sim.coefs[i,5]*as.numeric(repcw01$disaster[j]))       # 5
    ###   2   ###
