...
#data <- read.csv(file = "hyperlink or filename goes here", header = TRUE, sep = ",")
#or
# data <- read.dta(file = "hyperlink or filename goes here")
#or
...
```{r chunck name, echo=FALSE}


```

```{r loda data & write essential variables, echo=TRUE}
#descriptive statistics for all variables
#stargazer(voterid, type = 'text')
# run probit, show results
data <- na.omit(voterid)
...
```{r descriptive statistics stargazer, echo=TRUE, results='asis'}
...
```{r logit model - logit link, echo=TRUE}
# run probit, show results
...
```{r logit model stargazer - 2, echo=TRUE, results='asis'}
#summary(model_1l)
...
```{r logit predicted probabilities - automatic, echo=TRUE}
...
```{r logit test summary, echo=TRUE, results='asis'}
summary(data$pprob_test_l) # should be zeros
```

Calculate the average effects with a logit model.

```{r Calculate the average effect with logit, echo=TRUE}
...
```{r probit model - probit link, echo=TRUE}
# run probit, show results
(model_1p <- glm('photo ~ fraud + election_margin + gopleg', 
...
```{r probit model stargazer - 2, echo=TRUE, results='asis'}
#summary(model_1p)
...
```{r probit predicted probabilities - automatic, echo=TRUE}
# generate predicted probabilities automatically
...
```{r probit observed difference summary, echo=TRUE}
...
```{r set beta.hat, sigma.hat, and X.c -S, echo=TRUE}
...
```{r simulate p.tilde - S, echo=TRUE}
# simulate p.tilde
...
```{r characterize the simulations - S, echo=TRUE}
...
```{r violin plot of the simulation - V, echo=TRUE}
