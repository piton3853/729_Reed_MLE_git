...
model.1 <- lm(photo~fraud+election_margin+gopleg, data = voterid); summary(model.1) #stargazer(model.1, type = 'text')
plogis(predict(model.1))
model.1l <- glm(photo~fraud+election_margin+gopleg, data = voterid, family = 'binomial'(link='logit')); summary(model.1l)
plogis(predict(model.1l))
